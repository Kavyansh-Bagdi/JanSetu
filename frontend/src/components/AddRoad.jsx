...
function AddRoad({ mapRef }) {
  ...
  const submitRoadForm = async () => {
    ...
    const { name, location, builder_id, inspector_assigned, manager_id, status } = formData;

    if (!name || !builder_id || !inspector_assigned ) {
    ...
    const payload = {
      ...
      inspector_assigned: parseInt(formData.inspector_assigned, 10),
      ...
      {editingRoad && (
        <div className="fixed inset-0 flex items-center justify-center bg-transparent bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded shadow-lg w-96">
            ...
            {["name", "location", "builder_id", "inspector_assigned", "manager_id"].map((field) => (
            ...
            <label className="block mb-2">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, ended_date: e.target.value })}
                className="w-full border px-2 py-1 rounded mt-1"
              />
            </label>
